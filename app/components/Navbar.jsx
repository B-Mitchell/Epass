  ...
  return (
    <nav className='bg-[#1E1E1E] text-white w-full flex justify-between pb-3'>
        ...
        <ul className='hidden md:flex justify-between w-[50%] mt-5 mr-4'>
            <li className='cursor-pointer hover:text-[#FFC0CB] transition' onClick={() => {router.push('/')}}>Home</li>
            <li  className='cursor-pointer hover:text-[#FFC0CB] transition' onClick={() => {userId ? null : alert('please login'); router.push('/profile')}}>Profile</li>
            <li  className='cursor-pointer hover:text-[#FFC0CB] transition' onClick={() => {router.push('/events')}}>Events</li>
            <li  className={`cursor-pointer transition ${loginOut == 'logout' ? 'hover:text-red-600' : 'hover:text-[#FFC0CB]'}`} onClick={() => {handleLoginOut()}}>{loginOut}</li>
            {
                loginOut == 'login' ? 
                <li  className='cursor-pointer hover:text-[#FFC0CB] transition' onClick={() => router.push('/createAccount')}>Create Account</li> : null
            }
        </ul>
...

        {
         ...
         <ul className={`md:hidden block absolute m-0 w-[100%] mt-[4.56rem] bg-[#1E1E1E] text-center pb-5 z-50`}>
              <li className='cursor-pointer hover:text-[#FFC0CB] transition mt-3 text-[1.3rem]' onClick={() => {router.push('/') ; setIsActive(!active)}}>Home</li>
              <li  className='cursor-pointer hover:text-[#FFC0CB] transition mt-6 text-[1.3rem]' onClick={() => {userId ? null : alert('please login'); router.push('/profile'); setIsActive(!active)}}>Profile</li>
              <li  className='cursor-pointer hover:text-[#FFC0CB] transition mt-6 text-[1.3rem]' onClick={() => {router.push('/events'); setIsActive(!active)}}>Events</li>

              <li  className={`cursor-pointer transition mt-6 text-[1.3rem] ${loginOut == 'logout' ? 'hover:text-red-600' : 'hover:text-[#FFC0CB]'}`} onClick={() => {
                handleLoginOut(); 
                setIsActive(!active)}
                }>{loginOut}</li>
            {
                loginOut == 'login' ? 
                <li  className='cursor-pointer hover:text-[#FFC0CB] transition mt-6 text-[1.3rem] mb-5' onClick={() => {router.push('/createAccount'); setIsActive(!active)}}>Create Account</li> : null
            }
          </ul> : null
        }
    </nav>
